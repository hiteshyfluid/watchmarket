<x-main-layout>
    <section class="bg-gray-100 py-12">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 class="text-3xl font-bold text-gray-900">Upgrade Package Checkout</h1>
        </div>
    </section>
    ...
    <section class="py-12 bg-gray-50">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('seller.private.upgrade.checkout.process', [$advert, $level]) }}" class="space-y-6">
                @csrf
                ...
                <div class="bg-white border rounded p-6">
                    <div class="flex items-start justify-between gap-4">
                        ...
                        <div class="flex flex-col items-end gap-1 text-right whitespace-nowrap">
                            <a href="{{ route('seller.private.upgrade.packages', $advert) }}" class="text-sm text-blue-600 hover:underline">Change Package</a>
                            <a href="{{ route('my-account') }}" class="text-sm text-blue-600 hover:underline">Back to Dashboard</a>
                        </div>
                    </div>
                    <div class="mt-4 text-sm text-gray-700">
                        ...
                        @if($expiryDate)
                        <p>New Expiry Date: {{ \Carbon\Carbon::parse($expiryDate)->format('d F Y') }}</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white border rounded p-6 space-y-4">
                    ...
                    @if($requiresStripe)
                    <p class="text-sm text-gray-500">You will be redirected to Stripe {{ strtolower($stripeMode) }} checkout to pay securely by card. Your advert expiry will be updated only after Stripe confirms the payment.</p>
                    @else
                    <p class="text-sm text-gray-500">This package has no charge today. Submitting this form will upgrade your package immediately.</p>
                    @endif
                </div>

                <div>
                    <button type="submit" class="bg-black text-white font-bold py-3 px-8 uppercase tracking-widest hover:bg-gray-800 transition">
                        {{ $requiresStripe ? 'Continue to Stripe' : 'Upgrade Package' }}
                    </button>
                </div>
            </form>
        </div>
